...
| Change condition | Named facts that flip the outcome: jurisdiction moves onto the Call for Action list (33(1)(b) and 33(3A) engage), CDD becomes impossible under reg 28 (reg 31 makes the decision for the firm), a shell bank link surfaces (reg 34(2)), suspicion arises (51C(c) only if its reasonable-grounds and payment-service-nexus conditions are met, and the POCA route), mitigants stop being verifiable. |
...
3. **Mitigants and their effectiveness.** Merges the backlog's "available mitigants" and "control effectiveness". A mitigant that cannot be shown to work is not a mitigant. FATF Guidance para 142 (non-binding guidance) and FCTR 12.3.7G (FCA guidance).
...
- Overlay stage: PSRs 51B may not apply (a bank respondent is not a consumer, is not a micro-enterprise and is not a charity, so regulation 40(7) disapplication by agreement is possible if the parties have agreed it, and it is not established that the arrangement is a payment services framework contract; the £1 million income limit applies to charities only). PRIN 2A is not shown to apply (a bank respondent is not shown to be within "retail market business", and Principles 6 and 7 may apply instead; source pack C3). Stage 8 is contract notice, documentation and any SAR consent point. Do not assert the 51B position either way in the scenario feedback.
